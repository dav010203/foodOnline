...
def place_order(request):
    ...
    vendors_ids = []
    for i in cart_items:
        v_id = i.fooditem.vendor.id
        if v_id not in vendors_ids:
            vendors_ids.append(v_id)
...
def payments(request):
    if request.headers.get('x-requested-with') == 'XMLHttpRequest' and request.method == 'POST':
        ...
        to_emails = []
        for i in cart_items:
            email = i.fooditem.vendor.user.email
            if email not in to_emails:
                ...
                order_total = order_total_by_vendor(order, i.fooditem.vendor.id)
                context = {
                    'order': order,
                    'to_email': email,
                    'ordered_food_to_vendor': ordered_food_to_vendor,
                    'vendor_subtotal': order_total['subtotal'],
                    'tax_data': order_total['tax_dict'],
                    'vendor_grand_total': order_total['grand_total'],
                }
